  ...
  { 	
    if (!empty($_POST)){		
      ...
      if($result){   
        ...
        $this->session->set_flashdata('msg', '1');
        $this->session->set_flashdata('alert_data', 'Email Has Been Send With A Rest Link.');
        redirect('admin/forgot-password');       					
      }else{				
       $this->session->set_flashdata('msg', '2');
        $this->session->set_flashdata('alert_data', 'Invalid Email.');
        redirect('admin/forgot-password');
      }			
    }else{
  ...
  { 	
    if (!empty($_POST)){		
      ...
      if (!$this->form_validation->run() == FALSE){
        ...
        $this->general->update($data,$content);
        $this->session->set_flashdata('msg', '1');
        $this->session->set_flashdata('alert_data', 'Password Rest Successfull.');
        redirect('admin/login'); 
      }else{ 
